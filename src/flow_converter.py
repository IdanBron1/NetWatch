...
class FlowStats:
    ...
    def to_dict(self, key):
        ...
        # 1. Calculate Duration (with a tiny fallback to prevent dividing by zero)
        ...
        # 3. Calculate Totals for the new columns
        tot_pkts = self.fwd_pkts + self.bwd_pkts
        tot_bytes = self.fwd_bytes + self.bwd_bytes

        # 4. Calculate the new derived features that your Random Forest needs!
        avg_bytes_per_sec = tot_bytes / duration
        avg_pkts_per_sec = tot_pkts / duration
        flow_byts_s = avg_bytes_per_sec  # Usually the same thing in these datasets

        # Calculate packet ratio (Upload vs Download packets).
        # If no backward packets, fallback to a ratio of forward packets.
        pkt_ratio = self.fwd_pkts / self.bwd_pkts if self.bwd_pkts > 0 else float(self.fwd_pkts)
